...
bool HomeAssistantLightGroup::selectLightDetailAtIndex(int index) {
  ...
  auto output = static_cast<light::LightOutput*>(
...
void HomeAssistantLightGroup::toggleLight(int index) {
  ...
  if (!light->remote_values.is_on() && light->remote_values.get_brightness() == 0) {
  ...
  void HomeAssistantLightGroup::decBrightness() {
    ...
    if (activeLight) {
      ...
      if (brightness > 0) {
        ...
        ESP_LOGI(TAG, "'%s': brightness decreased to %f", activeLight->get_name().c_str(),
                brightness);
      } else {
        call.set_state(false);
        ESP_LOGI(TAG, "'%s': brightness decreased to off", activeLight->get_name().c_str());
      }
      call.perform();
    }
  }
  void HomeAssistantLightGroup::incBrightness() {
    ...
    if (activeLight) {
      ...
      if (activeLight->remote_values.is_on()) {
        ...
        ESP_LOGI(TAG, "'%s': brightness increased to %f", activeLight->get_name().c_str(),
                brightness);
      } else {
        call.set_state(true);
        call.set_brightness(0.1);
        ESP_LOGI(TAG, "'%s': brightness set to %f", activeLight->get_name().c_str(), 0.1);
      }
      call.perform();
    }
  }
  ...
  void HomeAssistantLightGroup::update_color_with_hsv(const float hsv_color, light::LightState* light) {
